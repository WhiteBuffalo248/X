...
BEGIN_MESSAGE_MAP(COutputList, CListBox)
	...
	ON_COMMAND(ID_VIEW_PROPERTIESWND, OnViewOutput)
...
void COutputBox::UpdateFonts()
{
	m_wndOutput.SetFont(&afxGlobalData.fontRegular);
}

void COutputBox::FillDebugWindow()
{
	m_wndOutput.AddString(_T("The OutputBox initial successfully."));
	m_wndOutput.AddString(_T(""));
}

void COutputBox::AdjusrHorzScroll(CListBox& wndListBox)
{
	CClientDC dc(this);
...
BEGIN_MESSAGE_MAP(COutputBox, CWnd)
	ON_WM_CREATE()
	ON_WM_SIZE()
	ON_WM_PAINT()
	ON_WM_SETFOCUS()
...
int COutputBox::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
...
void COutputBox::OnSize(UINT nType, int cx, int cy)
{
...
void COutputBox::OnSetFocus(CWnd* pOldWnd)
{
...
void COutputBox::OnLButtonDown(UINT nFlags, CPoint point)
{
	// TODO:  在此添加消息处理程序代码和/或调用默认值
	SetFocus();
