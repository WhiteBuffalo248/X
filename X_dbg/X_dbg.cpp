...
static void registercommands()
{
	...
	dbgcmdnew("pop", cbInstrPop, trueInitDebug
...
static DWORD WINAPI loadDbThread(LPVOID)
{
	...
	if (ErrorCodeInit(StringUtils::sprintf("%s\\preDefineCode\\errordb.txt", szProgramDir)))
	...
	if (ConstantCodeInit(StringUtils::sprintf("%s\\preDefineCode\\winconstants.txt", szProgramDir)))
...
BOOL WINAPI DllMain(HINSTANCE hInstDll, DWORD dwReason, PVOID ImpLoad)
{
	switch (dwReason)
	{
	default:
...
{
	p_stringConcatTaskThread_->setActive(false);
	ReleaseSemaphore(p_stringConcatTaskThread_->wakeupSemaphore, 1, nullptr);
	WaitForSingleObject(p_stringConcatTaskThread_->hThread, 500);
	dputs(QT_TRANSLATE_NOOP("DBG", "Stopping command thread..."), true);
	bStopCommandLoopThread = true;
	MsgFreeStack(gMsgStack);
	WaitForThreadTermination(hCommandLoopThread);
	dputs(QT_TRANSLATE_NOOP("DBG", "Stopping running debuggee..."), true);
	cbDebugStop(0, 0); //after this, debugging stopped
	dputs(QT_TRANSLATE_NOOP("DBG", "Aborting scripts..."), true);
	scriptabort();
	dputs(QT_TRANSLATE_NOOP("DBG", "Unloading plugins..."), true);
	pluginunloadall();
	dputs(QT_TRANSLATE_NOOP("DBG", "Cleaning up allocated data..."), true);
	cmdfree();
	//20200120
	varfree();
	yr_finalize();
	Zydis::GlobalFinalize();
	dputs(QT_TRANSLATE_NOOP("DBG", "Cleaning up wait objects..."), true);
	waitdeinitialize();
	SafeDbghelpDeinitialize();
	dputs(QT_TRANSLATE_NOOP("DBG", "Cleaning up debugger threads..."), true);
	dbgstop();
	dputs(QT_TRANSLATE_NOOP("DBG", "Saving notes..."), true);
	char* text = nullptr;
	...
	dputs(QT_TRANSLATE_NOOP("DBG", "Exit signal processed successfully!"), true);
#ifdef ENABLE_MEM_TRACE
	if (!memleaks())
		DeleteFileW(StringUtils::Utf8ToUtf16(alloctrace).c_str());
#endif //ENABLE_MEM_TRACE
