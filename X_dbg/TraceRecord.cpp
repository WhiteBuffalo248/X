		...
		if (rtFile != INVALID_HANDLE_VALUE)
		{
			...
			if (GetFileSizeEx(rtFile, &size))
			{
				...
				else //file is empty, write some file header
				{
					...
					json_t* j = json_string(path);
					json_object_set_new(root, "path", j);
					char* headerinfo;
					...
					int nLen = ::MultiByteToWideChar(CP_UTF8, NULL,
						headerinfo, headerinfosize, NULL, 0);
					/* 转换为Unicode */
					std::wstring wbuffer;
					wbuffer.resize(nLen);
					::MultiByteToWideChar(CP_UTF8, NULL, headerinfo, headerinfosize,
					...
					*/
					...
					WriteFile(rtFile, ansistr.data(), nLen, &written, nullptr);
					json_free(headerinfo);
					json_decref(root);
					if (written < (DWORD)nLen) //disk-full?
					{
						CloseHandle(rtFile);
						dputs(QT_TRANSLATE_NOOP("DBG", "Run trace failed to start because file header cannot be written."));
						return false;
